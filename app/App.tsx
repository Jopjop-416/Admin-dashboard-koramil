...
try {
  ...
  if (saved) {
    ...
    kecamatan.forEach((k) => {
      if (parsed[k.id]) {
        k.lat = parsed[k.id].lat;
        k.lng = parsed[k.id].lng;
      }
    });
  }
} catch (e) {}

export function updateKecamatanCoord(id: number, lat: number, lng: number) {
  const k = kecamatan.find((x) => x.id === id);
  if (k) {
    k.lat = lat;
    k.lng = lng;
    try {
      ...
      parsed[id] = { lat, lng };
      localStorage.setItem("kecamatan_coords", JSON.stringify(parsed));
    } catch (e) {}
  }
}

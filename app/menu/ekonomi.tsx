...
export default function Ekonomi() {
  ...
  const [sortKey, setSortKey] = useState<keyof KecamatanRow>("pdrb");
  ...
  const avgPdrb = Math.round(kecamatan.reduce((s, k) => s + k.pdrb, 0) / kecamatan.length);
  const totalMarkets = kecamatan.reduce((s, k) => s + k.markets, 0);
  const totalIndustry = kecamatan.reduce((s, k) => s + k.industry, 0);
  ...
  function exportCSV() {
    const header = ["No", "Kecamatan", "PDRB (ribu Rp)", "Pengangguran (%)", "Pasar", "Industri RT", "Koperasi", "Laju Pop (%)"];
    const rows = kecamatan.map((k, i) => [
      i + 1, k.name, k.pdrb, k.unemployment, k.markets, k.industry, k.cooperatives, k.growth
    ]);
    ...
    a.href = url;
    a.download = "data-ekonomi-lotim-2024.csv";
    a.click();
  ...
  const cols: { key: keyof KecamatanRow; label: string; fmt: (k: KecamatanRow) => React.ReactNode }[] = [
    ...
    { key: "pdrb", label: "PDRB (ribu Rp)", fmt: (k) => fmt(k.pdrb) },
    { key: "unemployment", label: "Pengangguran (%)", fmt: (k) => <span className={k.unemployment > 6 ? "text-red-500 font-semibold" : ""}>{k.unemployment.toFixed(1)}%</span> },
    { key: "markets", label: "Pasar", fmt: (k) => String(k.markets) },
    { key: "industry", label: "Industri RT", fmt: (k) => String(k.industry) },
    { key: "cooperatives", label: "Koperasi", fmt: (k) => String(k.cooperatives) },
    { key: "growth", label: "Laju Pop (%)", fmt: (k) => k.growth.toFixed(1) + "%" },
  ];
    ...
    <div className="px-6 pt-4 pb-6 space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-card border border-border rounded-xl p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-lg font-semibold font-sans text-foreground">Rp {fmtK(avgPdrb * 1000)}</p>
            <p className="text-[10px] text-muted-foreground">Rata-rata PDRB per Kapita</p>
          </div>
        </div>
        <div className="bg-card border border-border rounded-xl p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-lg font-semibold font-sans text-foreground">{totalMarkets}</p>
            <p className="text-[10px] text-muted-foreground">Total Pasar Tradisional & Modern</p>
          </div>
        </div>
        <div className="bg-card border border-border rounded-xl p-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-lg font-semibold font-sans text-foreground">{totalIndustry}</p>
            <p className="text-[10px] text-muted-foreground">Total Industri Rumah Tangga</p>
          </div>
        </div>
      </div>
